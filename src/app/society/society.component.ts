...
export class SocietyComponent implements OnInit {
  ...
  pakovanjeVestiZaKorisnika(): void {
    this.sveVesti.forEach(e => {
      if (e.visibility == "svi" || e.visibility == "svaUdruzenja" || e.creator == localStorage.getItem("ulogovan")) {
        this.sveVestiZaKorisnika.push(e);
      }
      else {
        if (e.visibility != "sviLovci") {
  ...
  ulogovanULocal(): void {
    this.users.forEach(element => {
      if (element.email == localStorage.getItem("ulogovan")) {
  ...
  postaviPodrazumevaneVrednosti(): void {
    ...
    this.numMembers = this.userPregledInfo.numMembers;
    this.membership = this.userPregledInfo.membership;
  }
  ...
  sacuvajIzmene(): void {
    ...
    if (this.proveraSviPodaciUneti() && this.proveraSviPodaciValidni() && this.proveraZaLoznike() && this.proveraZaTelefon()) {
      ...
      this.service.obrisiDatoUdruzenje(this.userPregledInfo.email).subscribe(user => {
        if (user['user'] == 'ok') {
          this.message = 'OK';
        }
      });
      ...
      this.newUser.surname = this.userPregledInfo.surname;
      this.newUser.middleName = this.userPregledInfo.middleName;
      this.newUser.birthdate = this.userPregledInfo.birthdate;
      ...
      this.newUser.email = this.userPregledInfo.email;
      ...
      this.newUser.animals = this.userPregledInfo.animals;
      this.newUser.favHuntGround = this.userPregledInfo.favHuntGround;
  ...
  popuniNizReklamaZaKorisnika():void{
    this.reklame.forEach(e => {
      if (e.visibility == "svi" || e.visibility == "svaUdruzenja") {
